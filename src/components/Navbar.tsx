...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className="bg-white dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800/90 z-40 px-4 py-2 flex-shrink-0 transition-colors">
      <div className="w-full flex flex-col md:flex-row items-center justify-between gap-2.5">
        {/* Top Left: Clean Title */}
        ...
        {/* Center: Graph On/Off Toggle Buttons */}
        <div className="flex items-center gap-1.5 bg-slate-100 dark:bg-slate-900/90 p-1 rounded-xl border border-slate-200 dark:border-slate-800 overflow-x-auto max-w-full">
          <span className="text-[10px] uppercase font-semibold text-slate-500 dark:text-slate-400 px-2 tracking-wider hidden lg:inline">
            Gráficos:
          </span>
          {graphToggles.map(({ key, label, icon: Icon }) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium whitespace-nowrap transition-all select-none ${
                  isEnabled
        ...
        {/* Right Tools: Light/Dark Mode Toggle with neutral gray icons */}
